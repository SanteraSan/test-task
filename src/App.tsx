...
function App() {
    ...
    useEffect(() => {
        setNodes(JSON.parse(localStorage.getItem('Changed nodes')) || []);
        setEdges(JSON.parse(localStorage.getItem('newEdges')) || []);
    }, []);
    ...
    const onNodesChangeInternal = useCallback((changes) => {
        ...
        localStorage.setItem('Changed nodes', JSON.stringify(changedNodes))
    }, [
        nodes,
        setNodes
    ]);
